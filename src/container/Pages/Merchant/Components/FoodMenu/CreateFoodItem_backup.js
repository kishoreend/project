...
const CreateFoodItem = () => {
  ...
  const [showadons, setshowAddons] = useState(false);
  ...
  const [drpAddOns, setAddOns] = useState([]);
  ...
  allCustomizeType.map((obj) => {
    //if (obj.type != "Add Ons" && obj.type != "Add-Ons" && obj.type != "AddOns" && obj.type != "addOns" && obj.type != "addons" && obj.type != "ADDONS" && obj.type != "ADD-ONS")
  ...
  function getaddonDrp_(array1) {
    console.log("add on ip", array1);
    console.log("add on label", array1.label);
    const drpaddons_new = [];
    array1.map((ip) => {
      console.log("addon ip inside", ip.label);
      customiseFoodItems_Latest.map((obj, index) => {
        if (ip.label == obj.type) {
          if (obj.type != "") {
            drpaddons_new.push({ value: index, label: obj.name });
          }
        }
      });
    });
    setallAdons(drpaddons_new);
      ...
      customiseFoodItems_Latest.map((obj, index) => {
        if (ip.label == obj.type) {
          if (obj.type != "") {
            ...
            drpcustomiseFoodItemEV.push({ value: index, label: obj.name });
          }
        }
      });
    });
  ...
  const CusType_PopulateFoodVariant = (a) => {
    console.log("values", a);
    setdrpNewFoodVariant(a);
    //setdrpNewExtraVariant(inputValue);
    //if (inputValue.length == 1) showFVariant(inputValue);
    if (a.length >= 1) setshowFV(true);
    if (a.length == 0) setshowFV(false);
    if (a.label == "Add-Ons" || a.label == "Add Ons" || a.label == "Add-Ons" || a.label == "AddOns" || a.label == "addOns" || a.label == "addons" || a.label == "ADDONS" || a.label == "ADD-ONS") {
      setshowAddons(true);
      getaddonDrp_(a);
    } else {
      setshowAddons(false);
          ...
          <div className="row mt-2 fullBorder">
            ...
            <div id="variants" className="col-lg-3 rightBorder ">
              ...
              <div className={showFV ? "p-2 fvheight" : "d-none"}>
                ...
                <div className="mb-1 d-none">Extra Variants</div>
            ...
            <div id="customisetype" className="rightBorder col-lg-3">
              ...
              <div className={showadons ? "p-2" : "d-none"}>
